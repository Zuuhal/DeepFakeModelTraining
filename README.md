...
- **audioTraining/**: Ses verileri üzerinde gerçekleştirilen özellik çıkarma (MFCC/LFCC), gürültü temizleme ve işitsel model eğitim süreçlerini içeren veri ön işleme ve eğitim kodları.
- **visualModelTraining/**: Görsel veriler üzerinde uygulanan kare çıkarma, normalizasyon ve veri artırma (augmentation) gibi ön işleme adımları ile hiperparametre optimizasyonlarını içeren eğitim scriptleri.
...
- Modeller arası tutarlılığı sağlamak adına, kullanılan her bir eğitim bloğu `visualModelArch` altındaki mimari yapılarla bağlantılıdır.
